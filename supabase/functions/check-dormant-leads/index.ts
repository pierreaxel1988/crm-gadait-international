...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch hot leads (Visit/Offre/Deposit) that are not won/lost/deleted
    const { data: hotLeads, error: leadsError } = await supabase
      .from('leads')
      .select('id, name, email, status, assigned_to, next_action_date, last_contact_date, pipeline_type, created_at')
    ...
    // Identify dormant leads: no recent action date and last contact > 7 days
    const dormantLeads = (hotLeads || []).filter(lead => {
      // If there's a future action scheduled, not dormant
      if (lead.next_action_date && new Date(lead.next_action_date) > new Date()) {
        return false;
      }

      // Check last contact date
      const lastActivity = lead.last_contact_date || lead.created_at;
    ...
    let emailsSent = 0;
    const emailAlerts: { leadName: string; status: string; agentName: string; daysSince: number; leadId: string }[] = [];
    ...
    for (const lead of dormantLeads) {
      ...
      // Check if notification already exists recently (avoid duplicates)
      if (lead.assigned_to) {
        ...
        const { error: notifError } = await supabase
          ...
          .insert({
            ...
            message: `Le lead "${lead.name}" (${lead.status}) n'a eu aucune action depuis ${daysSince} jours. Relancez-le rapidement !`,
            metadata: {
    ...
    // Send emails if there are dormant leads and Resend is configured
    if (emailAlerts.length > 0 && resendApiKey) {
      ...
      // Send individual email to each agent with their dormant leads
      for (const [agentName, alerts] of alertsByAgent.entries()) {
        ...
        const agentHtml = `
          <div style="font-family:'Helvetica Neue',Arial,sans-serif;max-width:640px;margin:0 auto;padding:20px;">
