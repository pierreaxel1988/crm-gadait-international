...
const PropertiesTabContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (transactionType !== 'all') {
      filtered = filtered.filter(property => {
        ...
        if (transactionType === 'rent') {
          return title.includes('location') || title.includes('louer') || title.includes('rent') || description.includes('location') || description.includes('louer') || description.includes('rent');
    ...
    if (selectedTypes.length > 0) {
      filtered = filtered.filter(property => property.property_type && selectedTypes.includes(property.property_type));
    }

    // Filtre par pays
    if (selectedCountries.length > 0) {
      filtered = filtered.filter(property => property.country && selectedCountries.some(country => property.country?.toLowerCase().includes(country.toLowerCase()) || country.toLowerCase().includes(property.country?.toLowerCase() || '')));
    }

    // Filtre par localisation
    if (selectedLocations.length > 0) {
      filtered = filtered.filter(property => property.location && selectedLocations.some(loc => property.location?.toLowerCase().includes(loc.toLowerCase()) || loc.toLowerCase().includes(property.location?.toLowerCase() || '')));
    }
  ...
  const handleSyncFromDatoCms = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('sync-datocms-properties', {
        body: {}
      });
      if (error) {
  ...
  const handleMigrateOldData = async () => {
    try {
      ...
      const convertedProperties = oldData.map((item: any) => ({
        ...
        images: [item['Main Image'], item['Secondary Image'], item['Additional Image 1'], item['Additional Image 2'], item['Additional Image 3'], item['Additional Image 4'], item['Additional Image 5']].filter(Boolean),
        features: [],
        amenities: [],
      ...
      <div className="flex items-start justify-between">
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 mt-6">
            ...
            {transactionType !== 'all' && <Badge className="bg-blue-50 border-blue-200 text-blue-700 font-futura rounded-md px-2 py-0.5 text-xs font-normal tracking-wide shadow-none hover:bg-blue-100 transition-all duration-200">
            ...
            {minBedrooms > 0}
            {(selectedTypes.length > 0 || selectedLocations.length > 0 || selectedCountries.length > 0 || priceRange[0] > 0 || priceRange[1] < 10000000) && <Badge className="bg-orange-50 border-orange-200 text-orange-700 font-futura rounded-md px-2 py-0.5 text-xs font-light tracking-wide shadow-none hover:bg-orange-100 transition-all duration-200">
        ...
        {/* Boutons d'action */}
        <div className="flex items-center gap-3">
          <Button onClick={handleClearAllProperties} disabled={clearing} variant="destructive" size="sm" className="font-futura">
            {clearing ? <>
          ...
          <Button onClick={handleMigrateOldData} disabled={migrating} variant="outline" size="sm" className="font-futura border-loro-pearl hover:bg-loro-white">
            {migrating ? <>
                <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
                Migration...
              </> : <>
          ...
          <Button onClick={handleSyncFromDatoCms} disabled={syncingDatoCms} className="bg-loro-sand text-loro-navy hover:bg-loro-sand/90 font-futura" size="sm">
            {syncingDatoCms ? <>
                <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
                Synchronisation...
              </> : <>
      ...
      <div className="space-y-4">
        <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <PropertySort currentSort={currentSort} onSortChange={setCurrentSort} />
          </div>
        </div>
        
        <PropertyFilters priceRange={priceRange} onPriceRangeChange={setPriceRange} selectedTypes={selectedTypes} onTypesChange={setSelectedTypes} selectedLocations={selectedLocations} onLocationsChange={setSelectedLocations} selectedCountries={selectedCountries} onCountriesChange={setSelectedCountries} transactionType={transactionType} onTransactionTypeChange={setTransactionType} minBedrooms={minBedrooms} onMinBedroomsChange={setMinBedrooms} isOpen={filtersOpen} onToggle={() => setFiltersOpen(!filtersOpen)} onClearFilters={clearAllFilters} />
      </div>
      
      {/* Contenu principal et pagination */}
      {filteredProperties.length === 0 && !loading ? <div className="text-center py-16 bg-gradient-to-br from-loro-white to-loro-pearl rounded-lg">
          ...
          {properties.length === 0 ? <>
              ...
              <div className="flex gap-4 justify-center">
                <Button onClick={handleMigrateOldData} disabled={migrating} variant="outline" className="font-futura border-loro-pearl hover:bg-loro-white">
                  {migrating ? <>
                      <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
                      Migration...
                    </> : <>
                ...
                <Button onClick={handleSyncFromDatoCms} disabled={syncingDatoCms} className="bg-loro-sand text-loro-navy hover:bg-loro-sand/90 font-futura">
                  {syncingDatoCms ? <>
                      <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
                      Synchronisation...
                    </> : <>
              ...
              <Button onClick={clearAllFilters} variant="outline" className="font-futura border-loro-sand text-loro-navy hover:bg-loro-sand hover:text-loro-navy">
