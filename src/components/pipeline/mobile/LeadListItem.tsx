...
const LeadListItem = ({
  ...
  return <div className={`py-3 px-4 flex items-center hover:bg-slate-50 transition-colors cursor-pointer ${nextFollowUpDate ? actionStyle.containerClassName : ''}`} onClick={() => onClick(id)}>
      <div className="mr-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center text-sm text-zinc-700 mt-1 gap-1">
          ...
          {taskType && <span className={`flex items-center ${nextFollowUpDate ? actionStyle.taskClassName : 'bg-zinc-100 text-zinc-700 rounded-full px-2 py-0.5'}`} onClick={taskType === 'Call' && phone ? handlePhoneCall : undefined} style={taskType === 'Call' && phone ? {
          cursor: 'pointer'
        } : undefined}>
              {taskType === 'Call' ? <Phone className={`h-3 w-3 mr-1 ${nextFollowUpDate ? actionStyle.iconClassName : 'text-zinc-600'}`} /> : taskType === 'Follow up' ? <Clock className={`h-3 w-3 mr-1 ${nextFollowUpDate ? actionStyle.iconClassName : 'text-zinc-600'}`} /> : taskType === 'Visites' ? <Calendar className={`h-3 w-3 mr-1 ${nextFollowUpDate ? actionStyle.iconClassName : 'text-zinc-600'}`} /> : null}
              <span className={`truncate text-xs ${nextFollowUpDate ? '' : 'text-zinc-900'}`}>
                {taskType}
              </span>
            </span>}
          
          {desiredLocation && <span className="flex items-center bg-zinc-100 text-zinc-700 rounded-full px-2 py-0.5">
              <MapPin className="h-3 w-3 mr-1 text-zinc-800" />
              <span className="truncate text-xs text-zinc-900">
                {desiredLocation}
              </span>
            </span>}
          
          {budget && <span className="truncate text-xs font-medium bg-zinc-100 text-zinc-900 rounded-full px-2 py-0.5">
