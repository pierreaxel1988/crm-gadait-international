    ...
    if (!open) {
      ...
      <DialogContent className="w-[95%] max-w-md p-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3 py-2">
          <FormInput label="Nom" name="name" value={leadName} onChange={e => setLeadName(e.target.value)} required />
          
          <FormInput label="Email" name="email" type="email" value={leadEmail} onChange={e => setLeadEmail(e.target.value)} required />
          
          <FormInput label="Téléphone" name="phone" value={leadPhone} onChange={e => setLeadPhone(e.target.value)} icon={Phone} />
          
          <FormInput label="Référence" name="reference" value={propertyRef} onChange={e => setPropertyRef(e.target.value)} />
          
          <FormInput label="Localisation" name="location" value={propertyLocation} onChange={e => setPropertyLocation(e.target.value)} icon={Flag} />
          
          <FormInput label="Type de propriété" name="propertyType" value={propertyType} onChange={e => setPropertyType(e.target.value)} icon={Home} />
          
          <FormInput label="Chambres" name="bedrooms" type="number" value={propertyBedrooms} onChange={e => setPropertyBedrooms(e.target.value)} />
          
          <div className="flex gap-2">
            <FormInput label="Budget" name="budget" type="text" value={budget} onChange={e => setBudget(e.target.value)} icon={Euro} className="flex-1" />
            
            <FormInput label="Devise" name="currency" value={currency} onChange={e => setCurrency(e.target.value)} className="w-24" />
          </div>
          
          <FormInput label="Nationalité" name="nationality" value={nationality} onChange={e => setNationality(e.target.value)} icon={Flag} />
        </div>
        ...
        <DialogFooter className="flex flex-row justify-between mt-4 gap-2 pt-2">
          ...
          <Button type="button" onClick={handleImport} disabled={isSubmitting || !leadName || !leadEmail} className="flex-1">
