...
const ActionCard: React.FC<ActionCardProps> = ({
  ...
  const formatDate = (dateStr: string | undefined) => {
    ...
    if (isToday(date)) {
      ...
      return format(date, 'EEEE à HH:mm', {
        locale: fr
      });
    } else {
  ...
  return <Card className={`p-4 transition-all cursor-pointer ${action.status === 'overdue' ? 'border-red-300 bg-[#FFDEE2]/30' : action.status === 'done' ? 'bg-gray-50/80 border-gray-200' : 'bg-[#F2FCE2]/40 border-green-100'}`} onClick={e => onCardClick(action.leadId, e)}>
      ...
      <div className="flex justify-between items-center mt-3">
        ...
        <div className="flex items-center gap-1">
          {action.phoneNumber && <>
              <Button variant="ghost" size="sm" className="h-8 w-8 p-0 rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200" onClick={e => {
            e.stopPropagation();
            if (action.phoneNumber) {
              const cleanedPhone = action.phoneNumber.replace(/[^\d+]/g, '');
              window.open(`https://wa.me/${cleanedPhone}`, '_blank');
          ...
          {action.email && <Button variant="ghost" size="sm" className="h-8 w-8 p-0 rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200" onClick={e => {
          e.stopPropagation();
          if (action.email) {
            window.location.href = `mailto:${action.email}`;
          }
          ...
          {action.status !== 'done' && <Button variant="outline" size="sm" className={getButtonClasses(action.status)} onClick={e => {
          e.stopPropagation();
          ...
          {action.status === 'done' && <span className="text-xs py-1 px-2 bg-green-100 text-green-700 rounded-full flex items-center">
