...
const TopAgentsTable: React.FC<TopAgentsTableProps> = ({
  ...
  const [tablePeriod, setTablePeriod] = useState<Period>({
    type: period === 'month' ? 'mois' : period === 'week' ? 'semaine' : period === 'year' ? 'annee' : 'mois'
  });
      ...
      try {
        ...
        await Promise.all(commercialTeamMembers.map(async member => {
          ...
          const {
            data: allLeads,
            error: leadsError
          } = await supabase.from('leads').select('id, status, budget').eq('assigned_to', member.id);
          if (leadsError) {
  ...
  const filteredAndSortedAgents = useMemo(() => {
    ...
    let filtered = commercialAgents.filter(agent => agent.name.toLowerCase().includes(searchTerm.toLowerCase()));
    if (sortBy) {
      filtered = [...filtered].sort((a, b) => {
        let valueA = sortBy === 'value' ? parseFloat(a[sortBy].replace(/[^0-9.-]+/g, '')) : a[sortBy];
        let valueB = sortBy === 'value' ? parseFloat(b[sortBy].replace(/[^0-9.-]+/g, '')) : b[sortBy];
