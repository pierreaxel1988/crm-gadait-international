    ...
    const fetchTeamMembers = async () => {
      try {
        const {
          data: members,
          error
        } = await supabase.from('team_members').select('id, name, role, email').eq('role', 'commercial').order('name');
        if (error) {
        ...
        const guaranteedAgents = getGuaranteedAgents().filter(agent => agent.role === 'agent' || agent.role === 'commercial');

        // Combine and remove duplicates
        const allMembers = [...(members || []), ...guaranteedAgents];
        const uniqueMembers = Array.from(new Map(allMembers.map(m => [m.id, m])).values()).sort((a, b) => a.name.localeCompare(b.name));
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (selectedAgent === "all") {
        ...
        if (unassignedLeads.length > 0) {
          ...
          agentData.push({
            ...
            // Keep the full text for unassigned
      ...
      agentData.push({
        name: "Total",
        firstName: "Total",
        // Keep the full text for total row
        stages: columnTotals,
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {[1, 2, 3, 4].map((_, rowIndex) => <TableRow key={rowIndex}>
                    <TableCell>
                      <Skeleton className="h-4 w-32" />
                    </TableCell>
                    {[1, 2, 3, 4, 5].map((_, colIndex) => <TableCell key={colIndex}>
