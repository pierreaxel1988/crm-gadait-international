...
import { MessageSquare, PieChart, Calendar, ListTodo, File, ClipboardList, ChevronLeft } from 'lucide-react';
    ...
    return <div className="sticky top-16 z-40 border-b border-loro-pearl bg-white shadow-sm">
        <div className="overflow-x-auto py-2">
          <div className="flex px-4 space-x-4 min-w-min">
            <button onClick={handleBack} className="flex flex-col items-center justify-center whitespace-nowrap rounded-md px-3 py-2 min-w-[70px] text-loro-navy hover:text-loro-terracotta hover:bg-loro-white/70">
              <ChevronLeft className="h-5 w-5 mb-1" />
              <span className="font-futura text-[14px]">Retour</span>
            </button>
        ...
        <NavigationMenu className="justify-start mx-auto">
          <NavigationMenuList className="space-x-0">
            <NavigationMenuItem>
              <button 
                onClick={handleBack}
                className="inline-flex h-12 items-center justify-center px-4 font-futura text-[18px] transition-colors hover:text-loro-terracotta focus:text-loro-terracotta focus:outline-none text-loro-navy"
              >
