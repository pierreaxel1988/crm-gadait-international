...
const ChatGadaitFloatingButton: React.FC<ChatGadaitFloatingButtonProps> = ({
  ...
  return <>
      {/* Floating Button */}
      {!isChatOpen && <div className={`fixed z-50 ${positionClasses[position]}`}>
          <Button onClick={() => setIsChatOpen(true)} size="lg" className="rounded-full h-14 w-14 shadow-lg text-white transition-all duration-200 hover:scale-110 bg-loro-terracotta">
            <MessageSquare size={24} />
          </Button>
        </div>}
      
      {/* Chat Modal */}
      {isChatOpen && <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4">
