  ...
  return <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Input id="name" value={formData.name || ''} onChange={handleInputChange} name="name" placeholder="Nom complet" className="w-full font-futura" required />
        </div>
      </div>

      {!hideSubmitButton && <div className="flex justify-end space-x-3 pt-2">
          <Button type="button" variant="outline" onClick={onCancel} disabled={isSubmitting}>
            Annuler
          </Button>
          <Button type="submit" disabled={isSubmitting} className="relative bg-gray-950 hover:bg-gray-800">
            {isSubmitting ? 'Sauvegarde en cours...' : lead ? 'Sauvegarder' : 'Créer'}
            {isSubmitting && <span className="absolute inset-0 flex items-center justify-center">
