...
const AssistantIA: React.FC<AssistantIAProps> = ({ leadId, lead, refresh }) => {
  ...
  const suggestions = [
    "Rédige une relance WhatsApp élégante",
    "Fais un résumé du client",
    "Propose trois biens adaptés"
  ];
      ...
      const response = await fetch("https://hxqoqkfnhbpwzkjgukrc.functions.supabase.co/chat-gadait", {
        ...
        body: JSON.stringify({
          ...
          leadContext: `
            ...
            Nombre de chambres: ${Array.isArray(lead.bedrooms) 
              ? lead.bedrooms.join(', ') 
  ...
  return (
    <div className="space-y-4 animate-[fade-in_0.3s_ease-out]">
      <div className="flex items-center gap-2 pb-2 border-b">
        <Sparkles className="h-4 w-4 text-loro-hazel" />
        <h3 className="text-sm font-futura uppercase tracking-wider text-gray-800">
          Assistant IA GADAIT
        </h3>
      </div>
      ...
      <div className="grid gap-2">
        {suggestions.map((suggestion, i) => (
          <Button
            key={i}
            variant="outline"
            className="w-full justify-start text-left font-normal hover:bg-loro-sand/10 border-loro-sand/60"
      ...
      <div className="space-y-2">
        <Textarea
          ...
          placeholder="Ex. Relance WhatsApp, résumé client..."
        />
        <Button
      ...
      {aiResponse && (
        <div className="rounded-lg border bg-card p-4 text-card-foreground animate-[fade-in_0.3s_ease-out]">
          <div className="flex items-start gap-2">
            <div className="h-6 w-6 rounded-full bg-loro-hazel/20 flex items-center justify-center">
              <Bot className="h-3 w-3 text-loro-hazel" />
            </div>
      ...
      {history.length > 0 && (
        <div className="rounded-lg border">
          <div className="px-4 py-2 border-b">
            <h4 className="text-xs text-muted-foreground uppercase">Historique des échanges</h4>
          </div>
