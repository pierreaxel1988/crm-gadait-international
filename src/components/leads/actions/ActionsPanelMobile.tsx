...
const ActionsPanelMobile: React.FC<ActionsPanelMobileProps> = ({
  ...
  const handleMarkComplete = async (action: ActionHistory) => {
    if (!lead) return;
    try {
      const updatedActionHistory = lead.actionHistory?.map(a => a.id === action.id ? {
        ...a,
  ...
  return <div className="space-y-6 py-[20px]">
      ...
      {pendingActions.length > 0 && <div className="space-y-3">
          <h4 className="font-medium text-chocolate-dark flex items-center">
            <Clock className="h-4 w-4 mr-2" />
            Actions en attente ({pendingActions.length})
          </h4>
          {pendingActions.map(action => <div key={action.id} className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center mb-2">
                    {getActionTypeIcon(action.actionType)}
                    <span className="ml-2 font-medium text-chocolate-dark">{action.actionType}</span>
                    {action.scheduledDate && <span className="ml-2 text-sm text-gray-600">
                        {format(new Date(action.scheduledDate), 'dd/MM/yyyy à HH:mm', {
                  locale: fr
                })}
                      </span>}
                  </div>
                  {action.notes && <p className="text-sm text-gray-700 mb-2">{action.notes}</p>}
                </div>
                <div className="flex space-x-1">
                  <Button onClick={() => handleMarkComplete(action)} size="sm" variant="outline" className="text-green-600 border-green-600 hover:bg-green-50">
                    <CheckCircle className="h-4 w-4" />
                  </Button>
                  <Button onClick={() => handleDeleteAction(action.id)} size="sm" variant="outline" className="text-red-600 border-red-600 hover:bg-red-50">
      ...
      {completedActions.length > 0 && <div className="space-y-3">
          <h4 className="font-medium text-chocolate-dark flex items-center">
            <CheckCircle className="h-4 w-4 mr-2" />
            Actions terminées ({completedActions.length})
          </h4>
          {completedActions.map(action => <div key={action.id} className="bg-green-50 border border-green-200 rounded-lg p-4">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center mb-2">
                    {getActionTypeIcon(action.actionType)}
                    <span className="ml-2 font-medium text-chocolate-dark">{action.actionType}</span>
                    {action.completedDate && <span className="ml-2 text-sm text-gray-600">
                        Terminé le {format(new Date(action.completedDate), 'dd/MM/yyyy à HH:mm', {
                  locale: fr
                })}
                      </span>}
                  </div>
                  {action.notes && <p className="text-sm text-gray-700">{action.notes}</p>}
                </div>
                <Button onClick={() => handleDeleteAction(action.id)} size="sm" variant="outline" className="text-red-600 border-red-600 hover:bg-red-50">
                  <Trash2 className="h-4 w-4" />
                </Button>
      ...
      {actionHistory.length === 0 && <div className="text-center py-8 text-gray-500">
