...
const Index = () => {
  return <div className="p-4 md:p-6 space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        <DashboardCard title="Lead Acquisition" subtitle="Last 7 months" icon={<BarChart3 className="h-5 w-5" />} className="lg:col-span-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        <DashboardCard title="Top Performing Agents">
          <div className="space-y-3 md:space-y-4">
            {['Sophie Martin', 'Thomas Bernard', 'Julie Dubois', 'Lucas Petit'].map((agent, index) => <div key={agent} className="flex items-center justify-between">
                ...
                <div>
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${index === 0 ? 'bg-luxury-200 text-luxury-800' : index === 1 ? 'bg-luxury-100 text-luxury-700' : 'bg-luxury-50 text-luxury-600'}`}>
                    #{index + 1}
                  </span>
                </div>
              </div>)}
        ...
        <DashboardCard title="Recent Conversions">
          <div className="space-y-3 md:space-y-4">
            {['Villa Saint-Tropez', 'Penthouse Paris 8e', 'Chalet Megève', 'Appartement Cannes Croisette'].map((property, index) => <div key={property} className="flex items-center justify-between">
                  <div className="flex items-center">
