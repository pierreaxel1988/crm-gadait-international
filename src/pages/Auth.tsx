    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-loro-white/80 px-4">
      ...
      <Card className="w-full max-w-md shadow-luxury">
        <CardHeader className="space-y-1 text-center">
          <CardTitle className="font-futura font-medium text-2xl text-loro-900">
          ...
          <CardDescription className="font-futura text-chocolate-light text-sm font-light">
            {isForgotPassword ? 'Entrez votre email pour recevoir un lien de réinitialisation' : isSignUp ? 'Créez votre compte pour accéder à toutes les fonctionnalités' : 'Connectez-vous pour accéder à votre espace personnel'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleAuth} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {!isSignUp && (
                  <div className="text-right">
                    <Button type="button" variant="link" onClick={() => setIsForgotPassword(true)} className="text-loro-terracotta text-xs p-0 h-auto">
                      Mot de passe oublié ?
                    </Button>
                  </div>
                )}
            ...
            <Button type="submit" disabled={loading} className="w-full text-white bg-loro-terracotta">
              {loading ? 'Chargement...' : isForgotPassword ? 'Envoyer le lien de réinitialisation' : isSignUp ? 'Créer un compte' : 'Se connecter'}
            </Button>
          </form>
          {!isForgotPassword && (
            <>
        ...
        <CardFooter className="text-center block">
          {isForgotPassword ? (
            <Button variant="link" onClick={() => setIsForgotPassword(false)} className="text-loro-terracotta">
