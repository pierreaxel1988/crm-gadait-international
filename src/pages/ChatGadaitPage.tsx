...
const ChatGadaitPage = () => {
  ...
  return <div className="flex flex-col h-[calc(100vh-8rem)] max-w-4xl mx-auto px-4">
      <div className="mb-4">
      ...
      <div className="flex-1 flex flex-col bg-loro-white rounded-lg shadow-luxury overflow-hidden">
        <ScrollArea className="flex-1 p-4">
          <div className="space-y-4 px-2">
            {messages.length === 0 ? <div className="flex h-full items-center justify-center py-12">
              ...
              </div> : messages.map(msg => <div key={msg.id} className={`max-w-[85%] ${msg.role === 'user' ? 'ml-auto' : 'mr-auto'}`}>
        ...
        <div className="p-4 border-t border-loro-pearl">
          <EnhancedInput input={input} setInput={setInput} placeholder="Ask anything..." isLoading={isLoading} handleSend={handleSendMessage} onKeyDown={handleKeyDown} />
        </div>
      </div>
    </div>;
};
